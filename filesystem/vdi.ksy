...
instances:
  ...
  blocks_map:
    ...
    type: block_ptr
    repeat: expr
    repeat-expr: header.header_main.blocks_in_image
    doc: >
      block_index = offset_in_virtual_disk / block_size
      actual_data_offset = blocks_map[block_index]+metadata_size+offset_in_block
...
types:
  ...
  block_ptr:
    ...
    instances:
      is_allocated:
        value: offset < _root.block_is_unallocated
      translated_offset:
        value: _root.header.blocks_offset+offset
        if: is_allocated
      block:
        io: _root._io
        pos: translated_offset
        type: block
        if: is_allocated
enums:
  image_type:
