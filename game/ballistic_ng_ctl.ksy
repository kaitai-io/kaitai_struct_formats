meta:
  id: ballisticng_ctl
  title: BallisticNG Custom Track Layout
  file-extension: ctl
  endian: le
  license: CC0-1.0
doc: |
  Format for custom race track layouts in BallisticNG.
  Only encodes non-decorative information about the track,
  such as spline control points and physics zones.
doc-ref: https://ballisticng-documentation.readthedocs.io/en/latest/ingame/layout_creator.html#ctl-custom-track-layout-specification
seq:
  - id: version
    type: s4
  - id: auto_junction_threshold
    type: f4
    doc: Official spec lists this as a bool, which seems to be a mistake.
  ...
  - id: routes
    type: route
    repeat: expr
    repeat-expr: num_routes
  - id: num_physics_zones
    type: s4
    if: version > 7
  - id: physics_zones
    type: physics_zone
    repeat: expr
    repeat-expr: num_physics_zones
    if: version > 7
types:
  route:
    seq:
      ...
      - id: control_points
        type: control_point
        repeat: expr
        repeat-expr: num_control_points
  control_point:
    seq:
      - id: pos
        type: vec3
